...
public:
  ...
  virtual void display() const { cout << "ID: " << id << " | Name: " << name; }
...
public:
  Patient(string n, string d, int i, string p, string diag, string adm,
          bool crit = false)
      : Person(n, d, i, p), diagnosis(diag), admissionDate(adm),
        lastWardName("None"), assignedWard(nullptr), critical(crit) {}

  Patient(const Patient &other)
      : Person(other), diagnosis(other.diagnosis),
  ...
  Patient(Patient &&other) noexcept
      : Person(std::move(other)), diagnosis(std::move(other.diagnosis)),
        admissionDate(std::move(other.admissionDate)),
        lastWardName(std::move(other.lastWardName)),
        assignedWard(other.assignedWard),
...
public:
  GeneralPractitioner(string n, string d, int i, string p, double s, string dep,
                      double fee)
...
public:
  Surgeon(string n, string d, int i, string p, double s, string dep,
          string spec, double fee)
      : HospitalStaff(n, d, i, p, s, dep), specialization(spec), perOpFee(fee) {
  }

  double calculateBillingRate() const override { return perOpFee; }
  void performOperation(Patient *p, string op) {
    p->addTreatment(Treatment(op, perOpFee, this));
  }
